#include "../../include/lib_base.h"
#include "../../include/ip_address.h"
#include "../../include/tcp_client.h"
#include "../../include/enum_classes.h"

int main(int argc , char *argv[])
{
  std::string input;
  int mode  = 0;
  std::stringstream msg;
  std::vector<std::string> message;

  if((argc != 4) || ((strcmp(argv[1],"v4") != 0) && (strcmp(argv[1],"v6") != 0)) )
  {
      std::cout << "[-] usage: " << argv[0] << " <mode> <address> <port>" << "\n";
      _exit (1);
  }

    mode = (strcmp(argv[1],"v4") == 0) ? AF_INET : AF_INET6;
    ip_address server_addr(argv[2] , argv[3] , mode , SOCK_STREAM );
    tcp_client countTCli(mode);

    countTCli.connect(server_addr);
    std::cout << "[+] Welcome to directoryClient!" << "\n";
    std::cout << "[+] Type 'quit' to exit!" << "\n";

    loop_always()
    {
      try
      {
        std::cout << "Insert surname you're looking for: > ";
        std::getline(std::cin , input);

        if(std::cin.eof())
        {
          std::cout << "CTRL-D" << "\n";
          break;
        }

        if(input.empty()) continue;

        CLEAR_MSG(msg);

        if(strcmp(input.c_str(),"quit") == 0)
        {
          msg << "CLOSE" << "\n"; /* CLOSE */
        }
        else
        {
          msg << "LOOKUP" << " " << input << "\n";  /* LOOKUP|surname */
        }

        countTCli.send_message(msg.str());
        message = countTCli.receive_message();

        if(strcmp(message[0].c_str(),"FOUND") == 0) /* FOUND|entry_num|list */
        {
          int _entries = std::atoi(message[1].c_str());
          if(_entries == 0 )
          {
            std::cout << "[-] No entry for given surname" << "\n";
          }
          else
          {
            for(int _iter=0 ; _iter < _entries; _iter++)
            {
              std::vector<char> raw_msg(message[_iter+2].begin() , message[_iter+2].end());
              std::replace_if(raw_msg.begin(), raw_msg.end(), ispunct, ' ');
              std::string _out(raw_msg.begin(), raw_msg.end());
              std::cout << "Found: " << _out << "\n";
            }
          }
        }
        else if(strcmp(message[0].c_str(),"ACK") == 0) /* ACK|OPT*/
        {
          if(strcmp(message[1].c_str(),"CLOSE") == 0) /* ACK|CLOSE*/
          {
            std::cout << "[-] Connection closed. Bye Bye! " << "\n";
            break;
          }
        }

  }
  catch(generic_exception & e)
 {
   switch(e.get_errcode())
   {
     case socket_status_t::BAD_READ:
       std::cout << "[-] Client Shutdown " << "\n";
       _exit (1);
   }
 }
 }
      return 0;
}
